...
export const Ajoneuvot: React.FC<Props> = () => {
        ...
        axios.get("https://tie.digitraffic.fi/api/v3/data/maintenance/trackings/latest")
            .then((response) => {
                response.data.features.map((feature: any, index: number) => {
                    vehicleTemp.push({ id: feature.properties.id, tasks: feature.properties.tasks, lat: feature.geometry.coordinates[1], lng: feature.geometry.coordinates[0] })
                });
    ...
    function handleResponse(message: any) {
        ...
        if (content.type === "Feature") {
            ...
            if (!match) {
                vehicleTemp.push({ id: content.properties.id, lat: content.geometry.coordinates[1], lng: content.geometry.coordinates[0], tasks: content.properties.tasks })
            }
        }
    ...
    const [polyline, setLine] = useState<PolyLine>({coords:[[0, 0]], color:"#FFFFF"})

    //Yhden ajoneuvon reitti
    function drawRoute(vehicleid:number, vehicletask:string){
        let tempLine:LatLngExpression[] = [];
        axios.get("https://tie.digitraffic.fi/api/v3/data/maintenance/trackings/" + vehicleid)
        .then((response) => {
            ...
            setLine({color: getTaskColor(vehicletask), coords:tempLine})
        });
    }
            ...
            <Marker key={index} position={[vehicle.lat, vehicle.lng]}>
                <Popup>
                    <div className='popup'>
                        ...
                        <h4>Tehtävät:</h4>
                        {tasks}
                        <button className='button content' onClick={() => drawRoute(vehicle.id, vehicle.tasks[0])}>Piirrä reitti</button>
                    </div>
                </Popup>
            </Marker>
        );
